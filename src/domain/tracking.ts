...
import { type Match, type Sweep, type WorldCupSnapshot, normalizeTeamName } from './sweep.js';
...
export function buildSweepTracking(sweep: Sweep, snapshot: WorldCupSnapshot, nations: Nation[]): SweepTracking {
  const nationByName = new Map(nations.map((nation) => [normalizeTeamName(nation.name), nation]));
  ...
  const trackedMatches = snapshot.matches.map((match) => toTrackedMatch(match, nationByName, ownerByTeam));
  ...
  const recentResults = trackedMatches
    .filter((match) => match.status === 'finished')
    ...
    const nextMatch = trackedMatches
      .filter(
        (match) =>
          ...
          (normalizeTeamName(match.homeTeam) === normalized || normalizeTeamName(match.awayTeam) === normalized)
      )
      .sort((left, right) => new Date(left.utcDate).getTime() - new Date(right.utcDate).getTime())[0];

    return {
      nation,
      status: eliminatedTeams.has(normalized) ? ('eliminated' as const) : ('active' as const),
      ownerName: ownerByTeam.get(normalized) ?? null,
      nextMatch: nextMatch ?? null
    };
  });
  const teamStatusByName = new Map(teamStatuses.map((status) => [normalizeTeamName(status.nation.name), status]));
  ...
  return {
    participants: sweep.participants.map((participant) => {
      ...
      const nextMatch = teams
        ...
        .sort((left, right) => new Date(left.utcDate).getTime() - new Date(right.utcDate).getTime())[0];
      const runSummary = getParticipantRunSummary(participant.teams, teams, trackedMatches, nextMatch ?? null);
...
function getParticipantRunSummary(
  ...
  if (activeCount > 0) {
    return nextMatch ? `Still alive - next ${shortRound(nextMatch.round)}` : 'Still alive';
  }

  const participantTeams = new Set(teamNames.map(normalizeTeamName));
  const furthestMatch = matches
    .filter((match) => participantTeams.has(normalizeTeamName(match.homeTeam)) || participantTeams.has(normalizeTeamName(match.awayTeam)))
    .sort((left, right) => roundRank(right.round) - roundRank(left.round) || new Date(right.utcDate).getTime() - new Date(left.utcDate).getTime())[0];

  return furthestMatch ? `Reached ${shortRound(furthestMatch.round)}` : 'Eliminated';
}
...
function toTrackedMatch(match: Match, nationByName: Map<string, Nation>, ownerByTeam: Map<string, string>): TrackedMatch {
  ...
  const homeParticipantName = ownerByTeam.get(normalizeTeamName(match.homeTeam)) ?? null;
  const awayParticipantName = ownerByTeam.get(normalizeTeamName(match.awayTeam)) ?? null;
  const participantNames = [...new Set([homeParticipantName, awayParticipantName].filter((name): name is string => Boolean(name)))];
...
function deriveEliminatedTeams(snapshot: WorldCupSnapshot, nations: Nation[]): Set<string> {
  const eliminated = new Set<string>();
  const knownTeams = new Set(nations.map((nation) => normalizeTeamName(nation.name)));

  for (const match of snapshot.matches) {
    if (match.status !== 'finished' || !match.winnerTeam || match.round.startsWith('Group ')) {
      continue;
    }

    const loser = normalizeTeamName(match.homeTeam) === normalizeTeamName(match.winnerTeam) ? match.awayTeam : match.homeTeam;
  ...
  const groupEliminations = deriveCompletedGroupEliminations(snapshot.matches, nations);
...
function deriveCompletedGroupEliminations(matches: Match[], nations: Nation[]): Set<string> {
  const eliminated = new Set<string>();
  ...
  const completedThirdPlace: Array<{ team: string; points: number; goalDifference: number; goalsFor: number }> = [];
  let completedGroups = 0;

  for (const [group, groupNations] of groups) {
    const groupNames = new Set(groupNations.map((nation) => normalizeTeamName(nation.name)));
    ...
    if (groupMatches.length !== 6 || groupMatches.some((match) => match.status !== 'finished')) {
      continue;
    }
  ...
  if (completedGroups === groups.size) {
    const thirdPlaceRanked = [...completedThirdPlace].sort(
      (left, right) => right.points - left.points || right.goalDifference - left.goalDifference || right.goalsFor - left.goalsFor
    );
